...
Assim, dada uma imagem $f(i, j)$, estando representada em escala de cinza, obtém-se uma imagem segmentada $g(i, j)$ conforme a equação abaixo, considerando um \textit{threshold} T dado:
...
% Assim, se uma imagem conter objetos com intensidades contrastantes, é possível realizar a limiarização. Para exemplificar, é mostrado na figura \ref{img:blns} uma imagem, sua versão em escala de cinza e seu histograma. 

% \begin{figure}[!htb]
% \centering
%     \caption{\label{img:blns} Imagem original, em escala de cinza e seu histograma.}
%     \subcaptionbox{Imagem original}{\includegraphics[scale=0.15]{img/balloons}}\qquad
%     \subcaptionbox{Escala de cinza}{\includegraphics[scale=0.15]{img/gray_balloons}}\qquad
%     \subcaptionbox{Histograma da imagem em escala de cinza}{\includegraphics[scale=0.12]{img/hist}}
%     \legend{(\textbf{Fonte: } Autor, 2019).}
% \end{figure}

% Nota-se uma maior concentração de pixels com intensidades mais baixas, que correspondem às partes escuras da imagem. A figura \ref{img:thr_blns} mostra o resultado da limiarização utilizando um \textit{threshold} muito baixo, alto e o ideal para separação total entre os balões e o plano de fundo.
...
\subsection{\textcolor{red}{Técnicas de inferência (Jorge comentou "verificar se todas as técnicas serão usadas")}}
